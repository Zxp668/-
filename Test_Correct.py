...
PI=3.14159
A=['wuxujiaoshui.wav','qinweiganhan.wav','yibanganhan.wav','yanzhongganhan.wav']
...
A_SPEED=0.20
A_huapen_thresh = 0.52#A赛道正常检测花盆的阈值
diyicizhuanwanqiandezhixing_time=3.5 #A区后区
diyicizhuanwanhoudezhixing_time=8.8 #A赛道正中间转向后，标准的直行时间
diyicizhuanwan_law=1.3#A区和B区第一个白色十字转弯角速度

#A区避障专用
#避障时的花盆检测阈值（小于正常花盆检测阈值，一般 = 正常阈值 - 横移阈值）
A_huapen_check_thresh = 0.35
...
B_SPEED=0.20
B_huapen_thresh = 0.40#B赛道正常检测花盆的阈值
diercizhuanwanqiandezhixing_time=3.0#B区后区
...
B_avoid_obstacle_action1_left_law = 1.5 #用于避障的角速度，略快，弥补曲线多余的时间耗费


#B_action_2为第一次转向后直行
#障碍物在左边用的参数
B_avoid_obstacle_action2_left_vel = 0.1 #用于避障的线速度，略快，弥补曲线多余的时间耗费
B_avoid_obstacle_action2_left_law = 0.0 #直行，角速度为零
B_avoid_obstacle_action2_left_time = 0.9#用于决定直行距离的执行时间
#障碍物在右边用的参数
B_avoid_obstacle_action2_right_vel = 0.1 #用于避障的线速度，略快，弥补曲线多余的时间耗费
B_avoid_obstacle_action2_right_law = 0.0 #直行，角速度为零
B_avoid_obstacle_action2_right_time = 1.0#用于决定直行距离的执行时间

#B_action_3为第二次转向
#障碍物在左边用的参数
B_avoid_obstacle_action3_left_vel = 0.0 #原地转向，线速度为0
B_avoid_obstacle_action3_left_law = 1.5 #用于避障的角速度，略快，弥补曲线多余的时间耗费

#B_action_4为第二次转向后直行
#障碍物在左边用的参数
B_avoid_obstacle_action4_left_vel = 0.15 #用于避障的线速度，略快，弥补曲线多余的时间耗费
B_avoid_obstacle_action4_left_law = 0.0 #直行，角速度为零
B_avoid_obstacle_action4_left_time = 3.6#用于决定直行距离的执行时间

#B_action_5为第三次转向
#障碍物在左边用的参数
B_avoid_obstacle_action5_left_vel = 0.0 #原地转向，线速度为0
B_avoid_obstacle_action5_left_law = 1.5 #用于避障的角速度，略快，弥补曲线多余的时间耗费

#B_action_6为第三次转向后直行
#障碍物在左边用的参数
B_avoid_obstacle_action6_left_vel = 0.1 #用于避障的线速度，略快，弥补曲线多余的时间耗费
B_avoid_obstacle_action6_left_law = 0.0 #直行，角速度为零
B_avoid_obstacle_action6_left_time = 0.9#用于决定直行距离的执行时间

#B_action_7为最后一次转向
#障碍物在左边用的参数
B_avoid_obstacle_action7_left_vel = 0.0 #原地转向，线速度为0
B_avoid_obstacle_action7_left_law = 1.5 #用于避障的角速度，略快，弥补曲线多余的时间耗费
...
class MOVE_ARRIVE:
    ...
    def A_huapen_dis_check(self):#避障过程中特殊的花盆检测函数
        ...
        if self.avoidance_direction == 1:#左侧有障碍物，检测右侧花盆距离
            if self.min_dis[1]<A_huapen_check_thresh and self.min_dis[1]!=0:#A区避障时检测阈值，小于正常花盆检测阈值，一般等于"正常阈值 - 横移阈值"
                #返回信号“检测到花盆！！！”
                return True
            else:
                return False
            ...
            #检测左侧花盆距离
            if self.min_dis[0]<A_huapen_check_thresh and self.min_dis[0]!=0:
                self.adapt_left_dis = self.min_dis[0]
                #返回信号“检测到花盆！！！”
                return True
            else:
                return False
    ...
    def A_arm_execute(self):
        ...
        if self.avoidance_c_time ==0 :
            ...
            self.time_delay(1.0)
            print(COLOR_YELLOW + ">>>>>>>>>>>Dry_Situation:" + str(
                self.area_A_dry[self.current_arm_index][0]) + STYLE_RESET)
            print(COLOR_YELLOW + ">>>>>>>>>>>Pump work. Times=" + str(self.area_A_dry[self.current_arm_index][0]) + STYLE_RESET)
            playsound(A[self.area_A_dry[self.current_arm_index][0]])
            ...
            self.time_delay(2.0)
            print(COLOR_YELLOW + ">>>>>>>>>>>Dry_Situation:" + str(
                self.area_A_dry[self.current_arm_index][1]) + STYLE_RESET)
            print(COLOR_YELLOW + ">>>>>>>>>>>Pump work. Times=" + str(self.area_A_dry[self.current_arm_index][1]) + STYLE_RESET)
            playsound(A[self.area_A_dry[self.current_arm_index][1]])
        ...
        if self.avoidance_direction >0 :#左侧障碍，原地右转
            while True:  # 永循环，直到误差abs(current_yaw-want_yaw)<2.5
                ...
                if current_yaw > 0.0:#0度朝向偏左，顺时针
                    self.cmd_vel_msg.angular.z = -(A_avoid_obstacle_action1_left_law-0.5)
                    self.cmd_vel_msg.linear.x = A_avoid_obstacle_action1_left_vel
                    self.cmd_vel_pub.publish(self.cmd_vel_msg)
                elif 0.0>=current_yaw >= want_yaw + 15.0:#0度朝向偏右，或转向过程中偏到目标角度范围正方向以外，顺时针转
                    self.cmd_vel_msg.angular.z = -A_avoid_obstacle_action1_left_law
                    self.cmd_vel_msg.linear.x = A_avoid_obstacle_action1_left_vel
                    self.cmd_vel_pub.publish(self.cmd_vel_msg)
                elif want_yaw + 15.0 > current_yaw >= want_yaw:#0度朝向偏右，或转向过程中偏到目标角度范围正方向以外，顺时针转
                    self.cmd_vel_msg.angular.z = -(A_avoid_obstacle_action1_left_law-0.5)
                    self.cmd_vel_msg.linear.x = A_avoid_obstacle_action1_left_vel
                    self.cmd_vel_pub.publish(self.cmd_vel_msg)
                elif want_yaw > current_yaw >= want_yaw - 15.0:#转向过程中偏到目标角度范围负方向以外，逆时针转
                    self.cmd_vel_msg.angular.z = A_avoid_obstacle_action1_left_law-0.5
                    self.cmd_vel_msg.linear.x = A_avoid_obstacle_action1_left_vel
                    self.cmd_vel_pub.publish(self.cmd_vel_msg)
                elif current_yaw <= want_yaw - 15.0:#转向过程中偏到目标角度范围负方向以外，逆时针转
                    self.cmd_vel_msg.angular.z = A_avoid_obstacle_action1_left_law
                    self.cmd_vel_msg.linear.x = A_avoid_obstacle_action1_left_vel
                    self.cmd_vel_pub.publish(self.cmd_vel_msg)
                else:#在want_yaw上下5度以内，开始原地微调，微调是顺时针还是逆时针？？？
                    #print("我在微调...")  # 验证是否会困于微调函数
                    self.cmd_vel_msg.angular.z = 0.0#此时原地微调
            ...
            while True:  # 永循环，直到误差abs(current_yaw-want_yaw)<2.5
                ...
                if current_yaw<0.0:#0度朝向偏右，逆时针
                    ...
                    self.cmd_vel_msg.linear.x = A_avoid_obstacle_action1_left_vel
                    self.cmd_vel_pub.publish(self.cmd_vel_msg)
                elif want_yaw - 15.0>=current_yaw >= 0.0:#0度朝向偏左，或转向过程中偏到目标角度范围负方向以外，逆时针转
                    self.cmd_vel_msg.angular.z = A_avoid_obstacle_action1_left_law
                    self.cmd_vel_msg.linear.x = A_avoid_obstacle_action1_left_vel
                    self.cmd_vel_pub.publish(self.cmd_vel_msg)
                elif want_yaw >=current_yaw >=want_yaw-15.0:#0度朝向偏左，或转向过程中偏到目标角度范围负方向以外，逆时针转
                    self.cmd_vel_msg.angular.z = A_avoid_obstacle_action1_left_law-0.5
                    self.cmd_vel_msg.linear.x = A_avoid_obstacle_action1_left_vel
                    self.cmd_vel_pub.publish(self.cmd_vel_msg)
                elif want_yaw + 15.0>current_yaw > want_yaw:#转向过程中偏到目标角度范围正方向以外，顺时针转
                    self.cmd_vel_msg.angular.z = -(A_avoid_obstacle_action1_left_law-0.5)
                    self.cmd_vel_msg.linear.x = A_avoid_obstacle_action1_left_vel
                    self.cmd_vel_pub.publish(self.cmd_vel_msg)
                elif current_yaw >= want_yaw + 15.0:#转向过程中偏到目标角度范围正方向以外，顺时针转
                    self.cmd_vel_msg.angular.z = -A_avoid_obstacle_action1_left_law
                    self.cmd_vel_msg.linear.x = A_avoid_obstacle_action1_left_vel
                    self.cmd_vel_pub.publish(self.cmd_vel_msg)
                else:#在want_yaw上下5度以内，开始原地微调，微调是顺时针还是逆时针？？？
    ...
    def A_avoid_action_3(self):#转回0度
        ...
        if self.avoidance_direction > 0:  # 左侧障碍
            while True:
                current_yaw, scope_law = self.read_imu_data()
                if current_yaw < want_yaw-15.0:
                    self.cmd_vel_msg.angular.z = A_avoid_obstacle_action3_left_law
                    self.cmd_vel_msg.linear.x = A_avoid_obstacle_action3_left_vel
                    self.cmd_vel_pub.publish(self.cmd_vel_msg)
                elif want_yaw > current_yaw >= want_yaw-15.0:
                    self.cmd_vel_msg.angular.z = A_avoid_obstacle_action3_left_law-0.5
                    self.cmd_vel_msg.linear.x = A_avoid_obstacle_action3_left_vel
                    self.cmd_vel_pub.publish(self.cmd_vel_msg)
                elif want_yaw+15.0>current_yaw > want_yaw:
                    self.cmd_vel_msg.angular.z = -(A_avoid_obstacle_action3_left_law-0.5)
                    self.cmd_vel_msg.linear.x = A_avoid_obstacle_action3_left_vel
                    self.cmd_vel_pub.publish(self.cmd_vel_msg)
                elif current_yaw > want_yaw+15.0:
                    self.cmd_vel_msg.angular.z = -A_avoid_obstacle_action3_left_law
                    self.cmd_vel_msg.linear.x = A_avoid_obstacle_action3_left_vel
                    self.cmd_vel_pub.publish(self.cmd_vel_msg)
                else:
    ...
    def A_avoid_action_5(self):#第三次转弯
        ...
        if self.avoidance_direction > 0:  # 左侧障碍，原地左转
            while True:  # 永循环，直到误差abs(current_yaw-want_yaw)<2.5
                current_yaw, scope_law = self.read_imu_data()
                if want_yaw-15.0 >= current_yaw > 0.0:  # 0度朝向偏左，顺时针
                    self.cmd_vel_msg.angular.z = A_avoid_obstacle_action5_left_law
                    self.cmd_vel_msg.linear.x = A_avoid_obstacle_action5_left_vel
                    self.cmd_vel_pub.publish(self.cmd_vel_msg)
                elif want_yaw >= current_yaw > want_yaw-15.0:  # 0度朝向偏左，顺时针
                    self.cmd_vel_msg.angular.z = A_avoid_obstacle_action5_left_law-0.5
                    self.cmd_vel_msg.linear.x = A_avoid_obstacle_action5_left_vel
                    self.cmd_vel_pub.publish(self.cmd_vel_msg)
                elif current_yaw < 0.0 :  # 0度朝向偏右，或转向过程中偏到目标角度范围正方向以外，顺时针转
                    self.cmd_vel_msg.angular.z = A_avoid_obstacle_action5_left_law-0.5
                    self.cmd_vel_msg.linear.x = A_avoid_obstacle_action5_left_vel
                    self.cmd_vel_pub.publish(self.cmd_vel_msg)
                elif want_yaw+15.0 >=current_yaw > want_yaw:  # 转向过程中偏到目标角度范围负方向以外，逆时针转
                    self.cmd_vel_msg.angular.z = -(A_avoid_obstacle_action5_left_law-0.5)
                    self.cmd_vel_msg.linear.x = A_avoid_obstacle_action5_left_vel
                    self.cmd_vel_pub.publish(self.cmd_vel_msg)
                elif current_yaw > want_yaw+15.0:  # 转向过程中偏到目标角度范围负方向以外，逆时针转
                ...
                if abs(current_yaw - want_yaw) < 2.5:
                    ...
                    continue
        elif self.avoidance_direction < 0:  # 右侧障碍，原地右转到90度
            while True:  # 永循环，直到误差abs(current_yaw-want_yaw)<2.5
                current_yaw, scope_law = self.read_imu_data()
                if want_yaw+15.0<current_yaw < 0.0:  # 0度朝向偏右，逆时针
                    self.cmd_vel_msg.angular.z = -A_avoid_obstacle_action5_left_law
                    self.cmd_vel_msg.linear.x = A_avoid_obstacle_action5_left_vel
                    self.cmd_vel_pub.publish(self.cmd_vel_msg)
                elif want_yaw+15.0>=current_yaw >want_yaw:  # 0度朝向偏右，逆时针
                    self.cmd_vel_msg.angular.z = -(A_avoid_obstacle_action5_left_law-0.5)
                    self.cmd_vel_msg.linear.x = A_avoid_obstacle_action5_left_vel
                    self.cmd_vel_pub.publish(self.cmd_vel_msg)
                elif current_yaw > 0.0 :  # 0度朝向偏左，或转向过程中偏到目标角度范围负方向以外，逆时针转
                    self.cmd_vel_msg.angular.z = -(A_avoid_obstacle_action5_left_law-0.5)
                    self.cmd_vel_msg.linear.x = A_avoid_obstacle_action5_left_vel
                    self.cmd_vel_pub.publish(self.cmd_vel_msg)
                elif want_yaw>=current_yaw > want_yaw-15.0:  # 转向过程中偏到目标角度范围正方向以外，顺时针转
                    self.cmd_vel_msg.angular.z = A_avoid_obstacle_action5_left_law-0.5
                    self.cmd_vel_msg.linear.x = A_avoid_obstacle_action5_left_vel
                    self.cmd_vel_pub.publish(self.cmd_vel_msg)
                elif want_yaw-15.0>=current_yaw > -180.0:  # 转向过程中偏到目标角度范围正方向以外，顺时针转
                    self.cmd_vel_msg.angular.z = A_avoid_obstacle_action5_left_law
                    self.cmd_vel_msg.linear.x = A_avoid_obstacle_action5_left_vel
                    self.cmd_vel_pub.publish(self.cmd_vel_msg)
                else:  # 在want_yaw上下5度以内，开始原地微调，微调是顺时针还是逆时针？？？
        ...
        if self.avoidance_direction > 0:  # 左侧障碍，原地右转
            while True:
                ...
                if current_yaw < want_yaw:
                    self.cmd_vel_msg.angular.z = A_avoid_obstacle_action7_left_law-0.5
                    self.cmd_vel_msg.linear.x = A_avoid_obstacle_action7_left_vel
                    self.cmd_vel_pub.publish(self.cmd_vel_msg)
                elif current_yaw > want_yaw+15:
                    self.cmd_vel_msg.angular.z = -A_avoid_obstacle_action7_left_law
                    self.cmd_vel_msg.linear.x = A_avoid_obstacle_action7_left_vel
                    self.cmd_vel_pub.publish(self.cmd_vel_msg)
                elif want_yaw+16>current_yaw > want_yaw:
                    self.cmd_vel_msg.angular.z = -(A_avoid_obstacle_action7_left_law-0.5)
                    self.cmd_vel_msg.linear.x = A_avoid_obstacle_action7_left_vel
                    self.cmd_vel_pub.publish(self.cmd_vel_msg)
                else:
            ...
            while True:
                current_yaw, scope_law = self.read_imu_data()
                if current_yaw < want_yaw-15:
                    self.cmd_vel_msg.angular.z = A_avoid_obstacle_action3_left_law
                    self.cmd_vel_msg.linear.x = A_avoid_obstacle_action3_left_vel
                    self.cmd_vel_pub.publish(self.cmd_vel_msg)
                elif want_yaw>current_yaw >want_yaw-16 :
                    self.cmd_vel_msg.angular.z = A_avoid_obstacle_action3_left_law - 0.5
                    self.cmd_vel_msg.linear.x = A_avoid_obstacle_action3_left_vel
                    self.cmd_vel_pub.publish(self.cmd_vel_msg)
    ...
    def B_arm_execute(self):#A区机械臂执行
        ...
        if self.avoidance_c_time ==0 :
            print("未进行避障")
            print(COLOR_YELLOW + ">>>>>>>>>>>Arm active first." + STYLE_RESET)
            if self.area_B_dry[self.current_arm_index][0] != 0:
                self.cmd_arm_msg.data = 9#标准情况下B赛道的向左摆动机械臂动作组
                self.cmd_arm_pub.publish(self.cmd_arm_msg)
                self.time_delay(1.0)
            print(COLOR_YELLOW + ">>>>>>>>>>>Dry_Situation:" + str(
                self.area_B_dry[self.current_arm_index][0]) + STYLE_RESET)
            print(COLOR_YELLOW + ">>>>>>>>>>>Pump work. Times=" + str(self.area_B_dry[self.current_arm_index][0]) + STYLE_RESET)
            if self.area_B_dry[self.current_arm_index][0] != 0:
                playsound(A[self.area_B_dry[self.current_arm_index][0]])
            ...
            if self.area_B_dry[self.current_arm_index][0] != 0:
                ...
                self.time_delay(5.0)
            print(COLOR_YELLOW + ">>>>>>>>>>>Arm active second." + STYLE_RESET)
            if self.area_B_dry[self.current_arm_index][1] != 0:
                self.cmd_arm_msg.data = 10#标准情况下B赛道的向右摆动机械臂动作组
                self.cmd_arm_pub.publish(self.cmd_arm_msg)
                self.time_delay(2.0)
            print(COLOR_YELLOW + ">>>>>>>>>>>Dry_Situation:" + str(
                self.area_B_dry[self.current_arm_index][1]) + STYLE_RESET)
            print(COLOR_YELLOW + ">>>>>>>>>>>Pump work. Times=" + str(self.area_B_dry[self.current_arm_index][1]) + STYLE_RESET)
            if self.area_B_dry[self.current_arm_index][1] != 0:
                playsound(A[self.area_B_dry[self.current_arm_index][1]])
    ...
    def B_avoid_action_1(self):  # 原地右转
        ...
        if self.avoidance_direction > 0:  # 左侧障碍，原地右转，目标90度
            while True:  # 永循环，直到误差abs(current_yaw-want_yaw)<2.5
                current_yaw, scope_law = self.read_imu_data()
                if 0.0 > current_yaw > -(180.0 + 15.0):  # 180度朝向偏左，顺时针
                    self.cmd_vel_msg.angular.z = -B_avoid_obstacle_action1_left_law
                    self.cmd_vel_msg.linear.x = B_avoid_obstacle_action1_left_vel
                    self.cmd_vel_pub.publish(self.cmd_vel_msg)
                elif -180 >= current_yaw >= -(180.0+15.0):  # 180度朝向偏左，顺时针
                    self.cmd_vel_msg.angular.z = -(B_avoid_obstacle_action1_left_law-0.5)
                    self.cmd_vel_msg.linear.x = B_avoid_obstacle_action1_left_vel
                    self.cmd_vel_pub.publish(self.cmd_vel_msg)
                elif  180.0 >= current_yaw >= want_yaw + 15.0:  # 180度朝向偏右，或转向过程中偏到目标角度范围正方向以外，顺时针
                    self.cmd_vel_msg.angular.z = -B_avoid_obstacle_action1_left_law
                    self.cmd_vel_msg.linear.x = B_avoid_obstacle_action1_left_vel
                    self.cmd_vel_pub.publish(self.cmd_vel_msg)
                elif  want_yaw + 15.0 > current_yaw > want_yaw :  # 180度朝向偏右，或转向过程中偏到目标角度范围正方向以外，顺时针
                    self.cmd_vel_msg.angular.z = -(B_avoid_obstacle_action1_left_law-0.5)
                    self.cmd_vel_msg.linear.x = B_avoid_obstacle_action1_left_vel
                    self.cmd_vel_pub.publish(self.cmd_vel_msg)
                elif want_yaw - 15.0 >= current_yaw >0:  # 转向过程中偏到目标角度范围负方向以外，逆时针转
                    self.cmd_vel_msg.angular.z = B_avoid_obstacle_action1_left_law
                    self.cmd_vel_msg.linear.x = B_avoid_obstacle_action1_left_vel
                    self.cmd_vel_pub.publish(self.cmd_vel_msg)
                elif want_yaw>= current_yaw >want_yaw - 15.0:  # 转向过程中偏到目标角度范围负方向以外，逆时针转
            ...
            while True:  # 永循环，直到误差abs(current_yaw-want_yaw)<2.5
                current_yaw, scope_law = self.read_imu_data()
                if 180.0>current_yaw > (180.0-15.0):# 180度朝向偏右，逆时针
                    self.cmd_vel_msg.angular.z = B_avoid_obstacle_action1_left_law-0.5
                    self.cmd_vel_msg.linear.x = B_avoid_obstacle_action1_left_vel
                    self.cmd_vel_pub.publish(self.cmd_vel_msg)
                elif (180.0-15.0)>current_yaw > 0:# 180度朝向偏右，逆时针
                    self.cmd_vel_msg.angular.z = B_avoid_obstacle_action1_left_law
                    self.cmd_vel_msg.linear.x = B_avoid_obstacle_action1_left_vel
                    self.cmd_vel_pub.publish(self.cmd_vel_msg)
                elif -180.0 < current_yaw <= want_yaw - 15.0:# 180度朝向偏右，或转向过程中偏到目标角度范围负方向以外，逆时针
                    self.cmd_vel_msg.angular.z = B_avoid_obstacle_action1_left_law
                    self.cmd_vel_msg.linear.x = B_avoid_obstacle_action1_left_vel
                    self.cmd_vel_pub.publish(self.cmd_vel_msg)
                elif want_yaw - 15.0 < current_yaw <= want_yaw :# 180度朝向偏右，或转向过程中偏到目标角度范围负方向以外，逆时针
                    self.cmd_vel_msg.angular.z = B_avoid_obstacle_action1_left_law-0.5
                    self.cmd_vel_msg.linear.x = B_avoid_obstacle_action1_left_vel
                    self.cmd_vel_pub.publish(self.cmd_vel_msg)
                elif want_yaw+15.0>=current_yaw >want_yaw:#转向过程中偏到目标角度范围正方向以外，顺时针转
                    ...
                    self.cmd_vel_msg.linear.x = B_avoid_obstacle_action1_left_vel
                    self.cmd_vel_pub.publish(self.cmd_vel_msg)
                elif 0>current_yaw >want_yaw+15.0:#转向过程中偏到目标角度范围正方向以外，顺时针转
                    self.cmd_vel_msg.angular.z = -B_avoid_obstacle_action1_left_law
                    self.cmd_vel_msg.linear.x = B_avoid_obstacle_action1_left_vel
                    self.cmd_vel_pub.publish(self.cmd_vel_msg)
                else:
    ...
    def B_avoid_action_3(self):  # 转回0度
        ...
        if self.avoidance_direction > 0:  # 左侧障碍
            ...
            while True:
                current_yaw, scope_law = self.read_imu_data()
                effective_current = current_yaw
                if current_yaw > 0:  # 当接近180°边界时
                    effective_current = current_yaw - 360  # 映射到负角度范围
                error = effective_current - want_yaw
                # 角度控制阈值 ±5°
                if error > 15.0:
                    # 需要顺时针转动
                    ...
                    # 需要顺时针转动
                    self.cmd_vel_msg.angular.z = -(B_avoid_obstacle_action3_left_law - 0.5)
                    self.cmd_vel_msg.linear.x = B_avoid_obstacle_action3_left_vel
                elif error < -15.0:
                    # 需要逆时针转动
                    self.cmd_vel_msg.angular.z = B_avoid_obstacle_action3_left_law
        ...
        if self.avoidance_direction > 0:  # 左侧障碍
            ...
            while True:
                ...
                if self.B_huapen_dis_check() and once_tag == 0:  # 如果避障过程检测到花盆,并且action4中尚未浇水#：停车并执行浇水
                    ...
                    print("once_tag = 1,表示action4中仅限一次的浇水机会用完了")
                    # !!!最后的判断相应变成if rospy.get_time() -Spend_Time - start_time1 > A_avoid_obstacle_action4_time
                if rospy.get_time() - t - start_time1 > B_avoid_obstacle_action4_left_time:  # 此段路程不用系数放大B_avoid_obstacle_action4_time
                    print("rospy.get_time() -t- start_time1 =", rospy.get_time() - t - start_time1)
                    print("B_avoid_obstacle_action4_time:", B_avoid_obstacle_action4_left_time)
                    print("rospy.get_time() -t- start_time1 > B_avoid_obstacle_action4_time")
                    print("跳出while循环")
    ...
    def B_avoid_action_5(self):  # 第三次转弯
        ...
        if self.avoidance_direction > 0:  # 左侧障碍
            while True:  # 永循环，直到误差abs(current_yaw-want_yaw)<2.5
                current_yaw, scope_law = self.read_imu_data()
                if 0.0 < current_yaw < 180.0-15.0:  # 180度朝向偏右，逆时针
                    self.cmd_vel_msg.angular.z = B_avoid_obstacle_action5_left_law
                    self.cmd_vel_msg.linear.x = B_avoid_obstacle_action5_left_vel
                    self.cmd_vel_pub.publish(self.cmd_vel_msg)
                if 180>=current_yaw >=180.0-15.0:  # 180度朝向偏右，逆时针
                    self.cmd_vel_msg.angular.z = B_avoid_obstacle_action5_left_law-0.3
                    self.cmd_vel_msg.linear.x = B_avoid_obstacle_action5_left_vel
                    self.cmd_vel_pub.publish(self.cmd_vel_msg)
                elif -180.0 < current_yaw <= want_yaw - 15.0:  # 180度朝向偏右，或转向过程中偏到目标角度范围负方向以外，逆时针
                    self.cmd_vel_msg.angular.z = B_avoid_obstacle_action5_left_law
                    self.cmd_vel_msg.linear.x = B_avoid_obstacle_action5_left_vel
                    self.cmd_vel_pub.publish(self.cmd_vel_msg)
                elif want_yaw> current_yaw > want_yaw - 15.0:  # 180度朝向偏右，或转向过程中偏到目标角度范围负方向以外，逆时针
                    self.cmd_vel_msg.angular.z = B_avoid_obstacle_action5_left_law-0.3
                    self.cmd_vel_msg.linear.x = B_avoid_obstacle_action5_left_vel
                    self.cmd_vel_pub.publish(self.cmd_vel_msg)
                elif want_yaw + 15.0>=current_yaw > want_yaw :  # 转向过程中偏到目标角度范围正方向以外，顺时针转
                    ...
                    self.cmd_vel_msg.linear.x = B_avoid_obstacle_action5_left_vel
                    self.cmd_vel_pub.publish(self.cmd_vel_msg)
                elif current_yaw < 0.0 and current_yaw >= want_yaw + 15.0:  # 转向过程中偏到目标角度范围正方向以外，顺时针转
                    self.cmd_vel_msg.angular.z = -B_avoid_obstacle_action5_left_law
                    self.cmd_vel_msg.linear.x = B_avoid_obstacle_action5_left_vel
                    self.cmd_vel_pub.publish(self.cmd_vel_msg)
                else:
            ...
            while True:  # 永循环，直到误差abs(current_yaw-want_yaw)<2.5
                ...
                if 0.0 > current_yaw > -(180.0 - 15.0):  # 180度朝向偏左，顺时针
                    ...
                    self.cmd_vel_msg.linear.x = B_avoid_obstacle_action5_left_vel
                    self.cmd_vel_pub.publish(self.cmd_vel_msg)
                if -(180.0+15.0)>= current_yaw > -180:  # 180度朝向偏左，顺时针
                    self.cmd_vel_msg.angular.z = -(B_avoid_obstacle_action5_left_law-0.3)
                    self.cmd_vel_msg.linear.x = B_avoid_obstacle_action5_left_vel
                    self.cmd_vel_pub.publish(self.cmd_vel_msg)
                elif 180.0 > current_yaw >= want_yaw + 15.0:  # 180度朝向偏右，或转向过程中偏到目标角度范围正方向以外，顺时针
        ...
        if self.avoidance_direction > 0:  # 左侧障碍
            ...
            while True:
                ...
                if error > 15.0:
                    ...
                    # 需要顺时针转动
                    self.cmd_vel_msg.angular.z = -(B_avoid_obstacle_action7_left_law-0.5)
                    self.cmd_vel_msg.linear.x = B_avoid_obstacle_action7_left_vel
                elif error < -15.0:
                    # 需要逆时针转动
                    self.cmd_vel_msg.angular.z = B_avoid_obstacle_action7_left_law
    ...
    def B_execute_avoidance_no_detect(self):  # 执行避障动作：写死，转向，越过，转回，摆正，可用class MOVE_ARRIVE:time_delay(self,s):函数强迫运行完动作组，再回归正确浇水前进
        """执行避障动作"""
        print("..................Execute_Avoidance..............")
        print("self.avoidance_direction",self.avoidance_direction)
        self.executing_avoidance = True  # 正在执行标志为挂起
        # 执行避障动作
        self.B_avoid_action_1()
        print("Action1结束")
    ...
    def A_dotime_0_015(self):#小车在A区直线行驶
        ...
        while True:
            ...
            # 如果避障过程没有浇水，下面程序正常运行，task_A()中i的循环进程无需变动
                ...
                self.cmd_vel_pub.publish(self.cmd_vel_msg)
                if A_huapen_thresh > self.min_dis[0] > self.side_obstacle_thresh+0.06:
                    print("检测到花盆")
                    self.adapt_left_dis = self.min_dis[0]
                    self.adapt_left_dis = self.min_dis[0]
                    self.adapt_left_dis = self.min_dis[0]
    ...
    def A_dotime_right_90(self):#小车在A区白色十字向右旋转刚好90度
        ...
        while True:#永循环，直到误差abs(current_yaw-want_yaw)<2.5
            ...
            if current_yaw>0.0:
                self.cmd_vel_msg.angular.z = -(diyicizhuanwan_law-0.5)
                self.cmd_vel_msg.linear.x = 0.0
                self.cmd_vel_pub.publish(self.cmd_vel_msg)
            elif 0.0>current_yaw>want_yaw+15.0:
    ...
    def A_B_dotime_right_90_015(self):#小车在A区白色十字到达B区第一个白色十字的直线行驶过程
        global want_yaw,current_yaw,start_time1,scope_law
        want_yaw = -90.0
        t = diyicizhuanwanhoudezhixing_time+(self.adapt_left_dis-0.36)*self.adapt_k0*self.adapt_k0_proportion+(self.min_dis[1]-0.36)*self.adapt_k1*self.adapt_k1_proportion#!!!!0.36为A赛道的左右两侧检测花盆的中值
        print("t",t)
        ...
        while True:
            ...
            self.cmd_vel_pub.publish(self.cmd_vel_msg)
            if rospy.get_time()-start_time1>t:#保证转弯完成，检测转弯时间是否超时
                break
            else:
                continue
    def B_dotime_right_180(self):#小车在B区第一个白色十字右转刚好90度，与初始角度成180度
        ...
        while True:#永循环，直到误差abs(current_yaw-want_yaw)<2.5
            ...
            if 0.0 > current_yaw > want_yaw+15.0:
                self.cmd_vel_msg.angular.z = -diyicizhuanwan_law
                self.cmd_vel_msg.linear.x = 0.0
                self.cmd_vel_pub.publish(self.cmd_vel_msg)
            elif want_yaw+15.0>=current_yaw>want_yaw:
                ...
                self.cmd_vel_msg.linear.x = 0.0  
                self.cmd_vel_pub.publish(self.cmd_vel_msg)
            if abs(current_yaw-want_yaw)<2.5:#不再精调，避免-179与179的角度突变
                self.cmd_vel_msg.angular.z = 0.0
                self.cmd_vel_msg.linear.x = 0.0
                self.cmd_vel_pub.publish(self.cmd_vel_msg)
                break
    ...
    def B_dotime_180_020(self):#小车在B区直线行驶
        ...
        while True:
            ...
            if t:  # 避障过程中浇水了，执行完避障动作后，以下程序不再执行，A_dotime_0_015函数到此为止，直接return，task_A()中i的循环进程需跳到下一次循环
                ...
                self.cmd_vel_pub.publish(self.cmd_vel_msg)
                if  B_huapen_thresh > self.min_dis[1] > self.side_obstacle_thresh+0.03:
                    print("检测到花盆")
                    print("Left : %f   Right : %f",self.min_dis[0],self.min_dis[1])
                    print("结束循环！！！！！！！")
                    break
    ...
    def task_A(self):
        ...
        for i in range(7):
            ...
            if i<3:
                if i == self.current_arm_index:#灌溉执行的标志位并没因避障过程浇水而+1
                    ...
                    while True:
                        print("卡task_A()在循环里了！！")
                        if A_huapen_thresh > self.min_dis[0] > self.side_obstacle_thresh:#双边检测
        ...
        print("self.obtacle_detected",self.obstacle_detected)
        ...
        for i in range(4):
            ...
            if i < 3:
                if i == self.current_arm_index:  # 灌溉执行的标志位并没因避障过程浇水而+1
                    ...
                    while True:
                        print("卡task_B()在循环里了！！")
                        if B_huapen_thresh > self.min_dis[1] > self.side_obstacle_thresh:#双边检测
                            print(COLOR_YELLOW + ">>>>>>>>>>>Stop." + STYLE_RESET)
                            self.dotime_stop()
                            break
                        else:
    ...
    def check_obstacles(self):  # 只负责实时设置标志位self.obstacle_detected，并配置避障动作所需的self.avoidance_direction，self.avoidance_start_time
        ...
        if self.area_left_min_dis < self.side_obstacle_thresh and self.area_left_min_dis>0.1:
            print("Left_Obstacle_Detected!!!")
            print("避障距离：Left: %.4f m,  Right: %.4f m"%(self.area_left_min_dis,self.area_right_min_dis))
        ...
        elif self.area_right_min_dis < self.side_obstacle_thresh and self.area_right_min_dis>0.1:
    ...
    def __init__(self):
        ...
        # 用于赛道之间适应性的调整横向直行时间（距离）
        self.adapt_left_dis = 0.0
        self.adapt_right_dis = 0.0
        self.adapt_k0 = -7.466#与左侧距离相关的系数
        self.adapt_k1 = 8.776# 与右侧距离相关的系数
        self.adapt_k0_proportion = 1.4#左侧距离相关系数的贡献比例
        self.adapt_k1_proportion = 0  #左侧距离相关系数的贡献比例
        ...
        # 障碍物检测阈值(单位:米)
        self.side_obstacle_thresh = 0.2255  # 障碍物检测阈值
        #避障所需标志位
        self.obstacle_detected = False  # 初始化为false，防止一运行就“检测到障碍物”
        self.executing_avoidance = False #初始化为false，表示避障动作是否正在执行
        #避障所需系数
        ...
        self.PID1 = PID(Kp=8.0, Ki=0.0, Kd=0.0)
        self.PID2 = PID(Kp=8.0, Ki=0.0, Kd=0.0)
        self.PID3 = PID(Kp=8.0, Ki=0.0, Kd=0.0)
    ...
    def draw_rectangles(self, columns, data_lists, title):
        ...
        绘制矩形图，支持A区和B区的布局
        columns: 矩形总列数（应为4列）
        data_lists: 包含两个区域的列表 [area_A_dry, area_B_dry]
        title: 图表标题
        """
        # 确保有正确的数据结构
        if len(data_lists) != 2 or columns != 4:
            raise ValueError("data_lists 应包含两个区域，且列数应为4")

        area_A_dry, area_B_dry = data_lists

        fig, ax = plt.subplots(figsize=(10, 6))
        ax.set_xlim(0, 8)  # 总宽度为8（4列×2）
        ax.set_ylim(0, 6)  # 高度为6
        ax.set_title(title)

        # 颜色映射：0-白色，1-绿色，2-蓝色，3-红色
        color_map = {0: "white", 1: "green", 2: "blue", 3: "red"}

        # 绘制A区（左两列）- 正确的行顺序
        for col in range(2):  # A区有两列
            for row in range(3):
                # 直接使用原始行索引：0=底部，1=中部，2=顶部
                value = area_A_dry[row][col]
                color = color_map.get(value, "gray")
                text_color = "black" if color == "white" else "white"

                # 计算位置：A区从X=0开始
                x_pos = col * 2
                y_pos = row * 2  # 行索引0在底部(y=0)，行索引2在顶部(y=4)

                rect = patches.Rectangle((x_pos, y_pos), 1.5, 1.5,
                                         color=color, edgecolor='black')
                ax.add_patch(rect)
                ax.text(x_pos + 0.75, y_pos + 0.75, str(value),
                        ha='center', va='center',
                        fontsize=10, color=text_color)

        # 绘制B区（右两列）- 保持原有顺序
        for col in range(2):  # B区有两列
            for row in range(3):
                # B区数据：特殊排列（从右到左，从上到下）
                # 列顺序反转：col=0 对应索引1，col=1 对应索引0
                value = area_B_dry[row][1 - col]
                color = color_map.get(value, "gray")
                text_color = "black" if color == "white" else "white"

                # 计算位置：B区从X=4开始
                x_pos = 4 + col * 2
                y_pos = (2 - row) * 2  # B区从上到下排列
        ...
        # 添加区域标签
        ax.text(1, 6.5, "Area_A", ha='center', va='center', fontsize=12)
        ax.text(5, 6.5, "Area_B", ha='center', va='center', fontsize=12)
    ...
    def RobotStart(self):
        ...
        try:
            ...
            # 用于省赛：在任务执行前绘制个矩形
            self.draw_rectangles(4, [self.area_A_dry, self.area_B_dry], "Initial Display of A, B")
            # 执行任务\
            playsound('kaishiyuyin.wav')
            self.task_A()
            #print("222")
            self.task_B() 
