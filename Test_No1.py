...
diyicizhuanwanqiandezhixing_time=1.5 #A区后区
diyicizhuanwanhoudezhixing_time=8.8 #A赛道正中间转向后，标准的直行时间
diyicizhuanwan_law=1.6#A区和B区第一个白色十字转弯角速度


B_SPEED=0.35
B_huapen_thresh = 0.40#B赛道正常检测花盆的阈值
diercizhuanwanqiandezhixing_time=1.7#B区后区
...
class MOVE_ARRIVE:
    ...
    def A_arm_execute(self):
        ...
        if self.area_A_dry[self.current_arm_index][0] !=0:
        ...
        print(COLOR_YELLOW + ">>>>>>>>>>>Pump work. Times=" + str(
            self.area_A_dry[self.current_arm_index][0]) + STYLE_RESET)
        ...
        if self.area_A_dry[self.current_arm_index][1] != 0:
    ...
    def A_dotime_0_015(self):#小车在A区直线行驶
        ...
        while True:
            ...
            # 如果避障过程没有浇水，下面程序正常运行，task_A()中i的循环进程无需变动
                ...
                self.cmd_vel_pub.publish(self.cmd_vel_msg)
                if A_huapen_thresh > self.min_dis[0] > self.side_obstacle_thresh+0.06 and A_huapen_thresh > self.min_dis[1] > self.side_obstacle_thresh+0.06:
                    print("检测到花盆")
    ...
    def A_dotime_0_015end(self):#小车在A区的最后一个花盆之后，十字之前的直线行驶
        ...
        while True:
            ...
            pid_output = self.PID1.update1(want_yaw,current_yaw)
            self.cmd_vel_msg.angular.z = pid_output
            self.cmd_vel_msg.linear.x = 0.35 #保证此速度与避障的直行速度相等，是下面执行时间的不等式具有有效性的前提
            self.cmd_vel_pub.publish(self.cmd_vel_msg)
    ...
    def A_dotime_right_90(self):#小车在A区白色十字向右旋转刚好90度
        ...
        while True:#永循环，直到误差abs(current_yaw-want_yaw)<2.5
            ...
            if current_yaw>0.0:
                self.cmd_vel_msg.angular.z = -(diyicizhuanwan_law-0.8)
                self.cmd_vel_msg.linear.x = 0.0
                self.cmd_vel_pub.publish(self.cmd_vel_msg)
    ...
    def A_B_dotime_right_90_015(self):#小车在A区白色十字到达B区第一个白色十字的直线行驶过程
        global want_yaw,current_yaw,start_time1,scope_law
        want_yaw = -90.0
        print("self.adapt_left_dis",self.adapt_left_dis)
        print("self.adapt_k0",self.adapt_k0)
        print("self.adapt_k0_proportion",self.adapt_k0_proportion)
        t = diyicizhuanwanhoudezhixing_time+(self.adapt_left_dis-0.36)*self.adapt_k0*self.adapt_k0_proportion#!!!!0.36为A赛道的左右两侧检测花盆的中值
        print("t",t)
    ...
    def B_dotime_180_020(self):#小车在B区直线行驶
        ...
        while True:
            ...
            if t:  # 避障过程中浇水了，执行完避障动作后，以下程序不再执行，A_dotime_0_015函数到此为止，直接return，task_A()中i的循环进程需跳到下一次循环
                ...
                self.cmd_vel_pub.publish(self.cmd_vel_msg)
                if  B_huapen_thresh > self.min_dis[1] > self.side_obstacle_thresh+0.03 and B_huapen_thresh > self.min_dis[0] > self.side_obstacle_thresh+0.03:
                    print("检测到花盆")
                    print("Left : %f   Right : %f",self.min_dis[0],self.min_dis[1])
                    print("结束循环！！！！！！！")
                    break
        ...
        for i in range(7):
            ...
            if i<3:
                if i == self.current_arm_index:#灌溉执行的标志位并没因避障过程浇水而+1
                    ...
                    while True:
                        print("卡task_A()在循环里了！！")
                        if A_huapen_thresh > self.min_dis[0] > self.side_obstacle_thresh+0.06 and A_huapen_thresh > self.min_dis[1] > self.side_obstacle_thresh+0.06:#双边检测
        ...
        for i in range(4):
            ...
            if i < 3:
                if i == self.current_arm_index:  # 灌溉执行的标志位并没因避障过程浇水而+1
                    ...
                    while True:
                        print("卡task_B()在循环里了！！")
                        if B_huapen_thresh > self.min_dis[1] > self.side_obstacle_thresh+0.03 and B_huapen_thresh > self.min_dis[0] > self.side_obstacle_thresh+0.03:#双边检测
                            print(COLOR_YELLOW + ">>>>>>>>>>>Stop." + STYLE_RESET)
                            self.dotime_stop()
                            break
                        else:
    ...
    def __init__(self):
        ...
        self.area_A_dry = [[2, 1],[3, 1], [3, 2]]#self.area_A_dry[self.current_arm_index][0]表示第...行小车左侧的花盆干旱程度
        self.area_B_dry = [[0, 0], [3, 2], [1, 1]]
        ...
        self.adapt_k0_proportion = 1.53#左侧距离相关系数的贡献比例
