...
#action_1为第一次转向
A_avoid_obstacle_action1_vel = 0.0 #原地转向，线速度为0
A_avoid_obstacle_action1_law = 1.5 #用于避障的角速度，略快，弥补曲线多余的时间耗费
#action_2为第一次转向后直行
A_avoid_obstacle_action2_vel = 0.15 #用于避障的线速度，略快，弥补曲线多余的时间耗费
A_avoid_obstacle_action2_law = 0.0 #直行，角速度为零
A_avoid_obstacle_action2_time = 1.1#用于决定直行距离的执行时间
#action_3为第二次转向
A_avoid_obstacle_action3_vel = 0.0 #原地转向，线速度为0
A_avoid_obstacle_action3_law = 1.5 #用于避障的角速度，略快，弥补曲线多余的时间耗费
#action_4为第二次转向后直行
A_avoid_obstacle_action4_vel = 0.15 #用于避障的线速度，略快，弥补曲线多余的时间耗费
A_avoid_obstacle_action4_law = 0.0 #直行，角速度为零
A_avoid_obstacle_action4_time = 3.8#用于决定直行距离的执行时间
#action_5为第三次转向
A_avoid_obstacle_action5_vel = 0.0 #原地转向，线速度为0
A_avoid_obstacle_action5_law = 1.5 #用于避障的角速度，略快，弥补曲线多余的时间耗费
#action_6为第三次转向后直行
A_avoid_obstacle_action6_vel = 0.1 #用于避障的线速度，略快，弥补曲线多余的时间耗费
A_avoid_obstacle_action6_law = 0.0 #直行，角速度为零
A_avoid_obstacle_action6_time = 1.5#用于决定直行距离的执行时间
#action_7为最后一次转向
A_avoid_obstacle_action7_vel = 0.0 #原地转向，线速度为0
A_avoid_obstacle_action7_law = 1.5 #用于避障的角速度，略快，弥补曲线多余的时间耗费
...
class MOVE_ARRIVE:
    ...
    # 避障过程中特殊的花盆检测函数
    def huapen_dis_check(self):#避障过程中特殊的花盆检测函数
        ...
        if self.avoidance_direction >0 :#左侧障碍，原地右转
            while True:  # 永循环，直到误差abs(current_yaw-want_yaw)<2.5
                ...
                if current_yaw > 0.0:#0度朝向偏左，顺时针
                    self.cmd_vel_msg.angular.z = -A_avoid_obstacle_action1_law
                    self.cmd_vel_msg.linear.x = A_avoid_obstacle_action1_vel
                    self.cmd_vel_pub.publish(self.cmd_vel_msg)
                    ...
                    self.cmd_vel_msg.angular.z = A_avoid_obstacle_action1_law#此时原地微调
            ...
            while True:  # 永循环，直到误差abs(current_yaw-want_yaw)<2.5
                ...
                if abs(current_yaw - want_yaw) < 2.5:
                    ...
                    continue
    def avoid_action_2(self):#第一次转弯后直行
        ...
        while True:
            ...
            self.cmd_vel_msg.linear.x = A_avoid_obstacle_action2_vel
            self.cmd_vel_pub.publish(self.cmd_vel_msg)
            if rospy.get_time() - start_time1 > (A_avoid_obstacle_action2_time*self.avoidance_c_time):#与放大后的时间比较
                break
            else:
                continue
    def avoid_action_3(self):#转回0度
        global want_yaw, current_yaw, scope_law
        want_yaw = 0
        ...
        while True:
            ...
            if current_yaw < want_yaw:
                self.cmd_vel_msg.angular.z = A_avoid_obstacle_action3_law
                self.cmd_vel_msg.linear.x = A_avoid_obstacle_action3_vel
                self.cmd_vel_pub.publish(self.cmd_vel_msg)
    ...
    def avoid_action_4(self):#第二次转弯后直行
        ...
        while True:
            ...
            pid_output = self.PID1.update1(want_yaw, current_yaw)  # PID1专用于A区浇水或避障的PID
            self.cmd_vel_msg.angular.z = pid_output
            self.cmd_vel_msg.linear.x = A_avoid_obstacle_action4_vel
            self.cmd_vel_pub.publish(self.cmd_vel_msg)

            # 如果避障过程中浇水了，必然占用直行时间，耽误直行距离
            # 解决方法：记录执行避障中处理花盆浇水的函数运行所需要的时间（运行了则在函数内部用两次rospy.get_time()标记时间起始点，没运行则时间为0.0s）

            if self.huapen_dis_check() and once_tag == 0:#如果避障过程检测到花盆,并且action4中尚未浇水#：停车并执行浇水
                #停车
                print("避障中检测到花盆！！！！")
                print("停车")
                self.dotime_stop()
            ...
            if rospy.get_time() -t- start_time1 > A_avoid_obstacle_action4_time :  # 此段路程不用系数放大A_avoid_obstacle_action4_time
                print("rospy.get_time() -t- start_time1 =",rospy.get_time() -t- start_time1)
                print("A_avoid_obstacle_action4_time:",A_avoid_obstacle_action4_time)
    ...
    def avoid_action_4_no_detect(self):#供最后一个花盆到转向点路段的避障，此段路径无需避障时检测花盆
        print("避障ing......Action4")  # 任务进程
        global want_yaw, current_yaw, start_time1, scope_law
        want_yaw = 0.0
        t=0.0 #存储：避障过程浇水占用了多少时间
        ...
        while True:
            ...
            self.cmd_vel_msg.linear.x = A_avoid_obstacle_action4_vel
            self.cmd_vel_pub.publish(self.cmd_vel_msg)
            if rospy.get_time() - start_time1 > A_avoid_obstacle_action4_time :  # 此段路程不用系数放大A_avoid_obstacle_action4_time
                print("rospy.get_time() - start_time1 =",rospy.get_time() - start_time1)
                print("A_avoid_obstacle_action4_time:",A_avoid_obstacle_action4_time)
                print("rospy.get_time() - start_time1 > A_avoid_obstacle_action4_time")
                print("跳出while循环")
                break
            else:
                continue
    ...
    def avoid_action_5(self):#第三次转弯
        ...
        if self.avoidance_direction < 0:  # 左侧障碍，原地右转
            while True:  # 永循环，直到误差abs(current_yaw-want_yaw)<2.5
                current_yaw, scope_law = self.read_imu_data()
                if current_yaw > 0.0:  # 0度朝向偏左，顺时针
                    self.cmd_vel_msg.angular.z = -A_avoid_obstacle_action5_law
                    self.cmd_vel_msg.linear.x = A_avoid_obstacle_action5_vel
                    self.cmd_vel_pub.publish(self.cmd_vel_msg)
                elif current_yaw < 0.0 and current_yaw >= want_yaw + 5.0:  # 0度朝向偏右，或转向过程中偏到目标角度范围正方向以外，顺时针转
                    self.cmd_vel_msg.angular.z = -A_avoid_obstacle_action5_law
                    self.cmd_vel_msg.linear.x = A_avoid_obstacle_action5_vel
                    ...
                    self.cmd_vel_msg.angular.z = A_avoid_obstacle_action5_law  # 此时原地微调
                    self.cmd_vel_msg.linear.x = 0.0  # 应该继续直行还是直接停住？？？
                    self.cmd_vel_pub.publish(self.cmd_vel_msg)
                if abs(current_yaw - want_yaw) < 2.5:
                    self.cmd_vel_msg.angular.z = 0.0
            ...
            while True:  # 永循环，直到误差abs(current_yaw-want_yaw)<2.5
                ...
                if abs(current_yaw - want_yaw) < 2.5:
                    ...
                    continue
    def avoid_action_6(self):  # 第三次转弯后的直行
        ...
        while True:
            ...
            pid_output = self.PID1.update1(want_yaw, current_yaw)  # PID1专用于A区浇水或避障的PID
            self.cmd_vel_msg.angular.z = pid_output
            self.cmd_vel_msg.linear.x = A_avoid_obstacle_action6_vel
            self.cmd_vel_pub.publish(self.cmd_vel_msg)
            if rospy.get_time() - start_time1 > (A_avoid_obstacle_action6_time * self.avoidance_c_time):  # 与放大后的时间比较
                break
            else:
                continue
    def avoid_action_7(self):
        global want_yaw, current_yaw, scope_law
        want_yaw = 0
        ...
        while True:
            ...
            if abs(current_yaw - want_yaw) < 2.5:
                ...
                continue
    def execute_avoidance(self):#执行避障动作：写死，转向，越过，转回，摆正，可用class MOVE_ARRIVE:time_delay(self,s):函数强迫运行完动作组，再回归正确浇水前进
         """执行避障动作"""
         ...
         t=self.avoid_action_4()#标志位，标志是否在避障过程检测到花盆并执行了浇水：检测花盆并执行则为Ture
    ...
    def execute_avoidance_no_detect(self):#供最后一个花盆到转向点路段的避障，此段路径无需避障时检测花盆
         """执行避障动作"""
         print("..................Execute_Avoidance..............")
         self.executing_avoidance = True#正在执行标志为挂起
         #执行避障动作
         ...
         #"Action4（直行部分），需要兼顾检测花盆
         self.avoid_action_4_no_detect()#供最后一个花盆到转向点路段的避障，此段路径无需避障时检测花盆
    ...
    def A_dotime_0_015(self):#小车在A区直线行驶
        ...
        while True:
            ...
            if self.obstacle_detected:
                print("!!!!检测到障碍物！！！！")
                t=self.execute_avoidance()#t为标志位，标志避障过程有没有浇水
                print("Irrigated？",t)
                continue#是不是有没有都一样？？？？？？
            ...
            # 如果避障过程没有浇水，下面程序正常运行，task_A()中i的循环进程无需变动
                ...
                self.cmd_vel_pub.publish(self.cmd_vel_msg)
                if self.min_dis[0] < 0.45 and self.min_dis[0]!=0.0:
                    print("...................结束本次直行.................")
    ...
    def A_dotime_0_015end(self):#小车在A区的最后一个花盆之后，十字之前的直线行驶
        ...
        while True:
            ...
            if self.obstacle_detected:
                ...
                t1 = rospy.get_time()
                print("获取起始时间", t1)
                self.execute_avoidance_no_detect()
                t2 = rospy.get_time()
                print("获取结束时间", t2)
                t=t2-t1
                print("避障占用时间", t)
                t_avoid = A_avoid_obstacle_action4_time
                continue  # 是不是有没有都一样？？？？？？
            pid_output = self.PID1.update1(want_yaw,current_yaw)
            self.cmd_vel_msg.angular.z = pid_output
            self.cmd_vel_msg.linear.x = A_avoid_obstacle_action4_vel#保证此速度与避障的直行速度相等，是下面执行时间的不等式具有有效性的前提
            self.cmd_vel_pub.publish(self.cmd_vel_msg)
            print("current_time:", rospy.get_time())
            print("t：", t)
            print("start_time:",start_time)
            print("Time_Costed（without avoidance）：", rospy.get_time()-start_time-t+t_avoid)
            if  rospy.get_time()-start_time-t+t_avoid>diyicizhuanwanqiandezhixing_time:#预计直行距离-避障直行距离
                print("Time_Costed（without avoidance）：", rospy.get_time()-start_time-t+t_avoid)
    ...
    def B_dotime_left_90(self):#小车在B区第二个白色十字左转刚好90度
        ...
        while True:
            current_yaw,scope_law=self.read_imu_data()
            if current_yaw>0.0 and current_yaw<180.0:#正向偏到179附近，逆时针转
                self.cmd_vel_msg.angular.z = diercizhuanwan_law
                self.cmd_vel_msg.linear.x = 0.0    
                self.cmd_vel_pub.publish(self.cmd_vel_msg)         
            elif current_yaw>-180.0 and current_yaw<=want_yaw-5.0:#正向偏到-179附近，或者转向过程中偏到目标角度范围负方向以外，逆时针转
                self.cmd_vel_msg.angular.z = diercizhuanwan_law
                self.cmd_vel_msg.linear.x = 0.0      
                self.cmd_vel_pub.publish(self.cmd_vel_msg)
