...
class MOVE_ARRIVE:
    ...
    def check_obstacles(self):
        ...
        # 左侧有障碍物(防止避障时撞到左侧)
        if self.area_left_min_dis < self.side_obstacle_thresh:
             print("Left_Obstecle_Detected!!!")
             self.obstacle_detected = True
             self.avoidance_direction = 1  # 向右微调
             self.avoidance_start_time = rospy.get_time()
    
         # 右侧有障碍物(防止避障时撞到右侧)
        elif self.area_right_min_dis < self.side_obstacle_thresh:
             print("Right_Obstecle_Detected!!!")
             self.obstacle_detected = True
         ...
         # 无障碍物时直行
             ...
             self.cmd_vel_msg.linear.x = 0.3  # 正常速度
             self.cmd_vel_msg.angular.z = 0.0
    ...
    def laser_callback(self, data):
        ...
        # 定义检测区域(角度范围)
        self.front_angles = list(range(0, 30)) + list(range(330, 360))
        self.left_angles = list(range(75, 105))
        self.right_angles = list(range(255, 285))
    
        # 获取各区域最小距离
        self.area_front_min_dis = min([data.ranges[i] for i in self.front_angles 
                             if not math.isnan(data.ranges[i])])
        self.area_left_min_dis = min([data.ranges[i] for i in self.left_angles 
                            if not math.isnan(data.ranges[i])])
        self.area_right_min_dis = min([data.ranges[i] for i in self.right_angles 
                             if not math.isnan(data.ranges[i])])
        check_obstacles()
    ...
    def execute_avoidance(self):
        ...
        # 根据避障方向设置速度
            self.cmd_vel_msg.linear.x = 0.1  # 避障时低速前进
            self.cmd_vel_msg.angular.z = 20 * self.avoidance_direction  # 转向angle
            self.cmd_vel_pub.publish(self.cmd_vel_msg)
