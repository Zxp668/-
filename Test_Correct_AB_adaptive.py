...
#这些变量 全部是给陀螺仪 / 加速度计串口解码用的
#要不要用 Serial（串口），其实取决于你现在的调试场景和性能需求。
buf_length = 11 #一帧数据长度 = 11 个字节

RxBuff = [0]*buf_length #创建一个列表，放 11 个 0
...
A_avoid_obstacle_action1_left_law = 1.2 #用于避障的角速度，略快，弥补曲线多余的时间耗费
#障碍物在右边用的参数
A_avoid_obstacle_action1_right_vel = 0.0 #原地转向，线速度为0
A_avoid_obstacle_action1_right_law = 1.2  #用于避障的角速度，略快，弥补曲线多余的时间耗费
...
A_avoid_obstacle_action2_right_time = 0.9#用于决定直行距离的执行时间

#A_action_3为第二次转向
#障碍物在左边用的参数
A_avoid_obstacle_action3_left_vel = 0.0 #原地转向，线速度为0
A_avoid_obstacle_action3_left_law = 1.2 #用于避障的角速度，略快，弥补曲线多余的时间耗费
#障碍物在右边用的参数
A_avoid_obstacle_action3_right_vel = 0.0 #原地转向，线速度为0
A_avoid_obstacle_action3_right_law = 1.2 #用于避障的角速度，略快，弥补曲线多余的时间耗费
...
A_avoid_obstacle_action5_left_law = 1.2 #用于避障的角速度，略快，弥补曲线多余的时间耗费
#A_障碍物在右边用的参数
A_avoid_obstacle_action5_right_vel = 0.0 #原地转向，线速度为0
A_avoid_obstacle_action5_right_law = 1.2 #用于避障的角速度，略快，弥补曲线多余的时间耗费

#A_action_6为第三次转向后直行
#障碍物在左边用的参数
A_avoid_obstacle_action6_left_vel = 0.1 #用于避障的线速度，略快，弥补曲线多余的时间耗费
A_avoid_obstacle_action6_left_law = 0.0 #直行，角速度为零
A_avoid_obstacle_action6_left_time = 1.6#用于决定直行距离的执行时间
#障碍物在右边用的参数
A_avoid_obstacle_action6_right_vel = 0.1 #用于避障的线速度，略快，弥补曲线多余的时间耗费
A_avoid_obstacle_action6_right_law = 0.0 #直行，角速度为零
A_avoid_obstacle_action6_right_time = 1.6#用于决定直行距离的执行时间

#A_action_7为最后一次转向
#障碍物在左边用的参数
A_avoid_obstacle_action7_left_vel = 0.0 #原地转向，线速度为0
A_avoid_obstacle_action7_left_law = 1.2 #用于避障的角速度，略快，弥补曲线多余的时间耗费
#障碍物在右边用的参数
A_avoid_obstacle_action7_right_vel = 0.0 #原地转向，线速度为0
A_avoid_obstacle_action7_right_law = 1.2 #用于避障的角速度，略快，弥补曲线多余的时间耗费


B_SPEED=0.20
B_huapen_thresh = 0.5#B赛道正常检测花盆的阈值
...
B_avoid_obstacle_action1_left_law = 1.0 #用于避障的角速度，略快，弥补曲线多余的时间耗费
#障碍物在右边用的参数
B_avoid_obstacle_action1_right_vel = 0.0 #原地转向，线速度为0
B_avoid_obstacle_action1_right_law = 1.0 #用于避障的角速度，略快，弥补曲线多余的时间耗费
...
B_avoid_obstacle_action2_right_time = 0.9#用于决定直行距离的执行时间

#B_action_3为第二次转向
#障碍物在左边用的参数
B_avoid_obstacle_action3_left_vel = 0.0 #原地转向，线速度为0
B_avoid_obstacle_action3_left_law = 1.0 #用于避障的角速度，略快，弥补曲线多余的时间耗费
#障碍物在右边用的参数
B_avoid_obstacle_action3_right_vel = 0.0 #原地转向，线速度为0
B_avoid_obstacle_action3_right_law = 1.0 #用于避障的角速度，略快，弥补曲线多余的时间耗费
...
B_avoid_obstacle_action6_left_time = 1.2#用于决定直行距离的执行时间
#障碍物在右边用的参数
B_avoid_obstacle_action6_right_vel = 0.1 #用于避障的线速度，略快，弥补曲线多余的时间耗费
B_avoid_obstacle_action6_right_law = 0.0 #直行，角速度为零
B_avoid_obstacle_action6_right_time = 1.2#用于决定直行距离的执行时间

#B_action_7为最后一次转向
#障碍物在左边用的参数
B_avoid_obstacle_action7_left_vel = 0.0 #原地转向，线速度为0
B_avoid_obstacle_action7_left_law = 1.0 #用于避障的角速度，略快，弥补曲线多余的时间耗费
#障碍物在右边用的参数
B_avoid_obstacle_action7_right_vel = 0.0 #原地转向，线速度为0
B_avoid_obstacle_action7_right_law = 1.0 #用于避障的角速度，略快，弥补曲线多余的时间耗费
...
class MOVE_ARRIVE:
    ...
    def A_arm_execute(self):
        ...
        if self.avoidance_c_time ==0 :
            ...
            playsound(A[self.area_A_dry[self.current_arm_index][1] - 1])
        ...
        if self.avoidance_direction > 0:  # 左侧障碍，原地右转
            ...
            while True:
                ...
                if current_yaw < want_yaw-15:
                    ...
                    self.cmd_vel_msg.angular.z = A_avoid_obstacle_action3_right_law - 0.5
                    self.cmd_vel_msg.linear.x = A_avoid_obstacle_action3_right_vel
                    self.cmd_vel_pub.publish(self.cmd_vel_msg)
                elif current_yaw > want_yaw:
                    self.cmd_vel_msg.angular.z = -(A_avoid_obstacle_action3_right_law-0.5)
    ...
    def __init__(self):
        ...
        self.adapt_k0_proportion = 1.3#左侧距离相关系数的贡献比例
        ...
        # 障碍物检测阈值(单位:米)
        self.side_obstacle_thresh = 0.22  # 障碍物检测阈值
        #避障所需标志位
        self.obstacle_detected = False  # 初始化为false，防止一运行就“检测到障碍物”
        self.executing_avoidance = False #初始化为false，表示避障动作是否正在执行
        #避障所需系数
